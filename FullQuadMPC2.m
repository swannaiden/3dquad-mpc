...
% dx(10) = dx(10)+g;
% dx = dx';
% f = @(x,u) [x(8);x(9);x(10);1/2*[0 -u(2) -u(3) -u(4); u(2) 0 -u(4) -u(3); u(3) -u(4) 0 u(2); u(4) -u(3) -u(2) 0]*[x(4);x(5);x(6);x(7)];...
%     quatrotate([x(4);x(5);x(6);x(7)]', [0,0,u(1)])/m]+[0,0,0,0,0,0,0,0,0,g];
%f = @dynamics;

f = @(x,u) [x(8:10);1/2*[0 -u(2) -u(3) -u(4); u(2) 0 -u(4) -u(3); u(3) -u(4) 0 u(2); u(4) -u(3) -u(2) 0]*x(4:7)...
    ;(squatrotate(x(4:7)', [0,0,u(1)])/m)'];

% % dynamics(x0,u)
% X = opti.variable(10,1); % state trajectory
% U = opti.variable(4,1);
% dynamics(X,U);
...
quat0 = eul2quat([0,0,0]);
x0 = [0,0,0,quat0(1), quat0(2), quat0(3), quat0(4),0,0,0]';
opti.subject_to(X(1:10,1)==x0);
